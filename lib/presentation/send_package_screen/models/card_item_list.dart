      ...
      children: cardItems.asMap().entries.map((entry) {
        ...
        return GestureDetector(
          onTap: () async {
            setState(() {
              selectedIndex = index;
              selectedCardItemName = cardItem.name;
            });
            ...
            await _prefs?.setString('selectedCardItemName', selectedCardItemName ?? '');
          },
          child: Card(
            elevation: selectedIndex == index ? 8 : 6, // Adjust elevation for highlighted item
            child: ListTile(
              tileColor: selectedIndex == index ? ColorConstant.highlighter.withOpacity(0.2) : ColorConstant.gray50, // Background color for highlighted item
